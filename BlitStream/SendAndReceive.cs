
using System;
using System.Net;
using System.Net.Sockets;
using System.Collections.Generic;
using System.Threading;
using System.IO;
using System.Runtime.Serialization.Formatters.Binary;

namespace BlitzBit {

    public partial class BlitStream {

        public void Send (int packetId, byte[] data) {

            byte[] buffer = new byte[1];
            Queue<byte> sendStream = new Queue<byte>();

            foreach (byte b in BitConverter.GetBytes(data.Length))
                sendStream.Enqueue(b);

            foreach (byte b in BitConverter.GetBytes((ushort)packetId))
                sendStream.Enqueue(b);

            foreach (byte b in data)
                sendStream.Enqueue(b);

            while (sendStream.Count != 0) {

                mutex.WaitOne(); try {

                    buffer[0] = sendStream.Dequeue();
                    stream.Write(buffer, 0, 1);

                } finally { mutex.ReleaseMutex(); }
            }
        }

        public void SendT (int packetId, object obj) {

            BinaryFormatter binaryFormatter = new BinaryFormatter();
            MemoryStream memoryStream = new MemoryStream();

            binaryFormatter.Serialize(memoryStream, obj);

            byte[] data = memoryStream.ToArray();

            byte[] buffer = new byte[1];
            Queue<byte> sendStream = new Queue<byte>();

            foreach (byte b in BitConverter.GetBytes(data.Length))
                sendStream.Enqueue(b);

            foreach (byte b in BitConverter.GetBytes((ushort)packetId))
                sendStream.Enqueue(b);

            foreach (byte b in data)
                sendStream.Enqueue(b);

            while (sendStream.Count != 0) {

                mutex.WaitOne(); try {

                    buffer[0] = sendStream.Dequeue();
                    stream.Write(buffer, 0, 1);

                } finally { mutex.ReleaseMutex(); }
            }
        }

        public void Recv (Action<int, byte[]> onRecv) {

            byte[] buffer = new byte[1];
            int recvByteCount = 0, packetLength = -1, packetId = -1;
            List<byte> recvBuffer = new List<byte>();

            while (true) {

                mutex.WaitOne(); try {

                    recvByteCount = stream.Read(buffer, 0, 1);
                    recvBuffer.Add(buffer[0]);

                    if (recvByteCount == 0) Close();

                    if (packetLength == -1) {

                        if (recvBuffer.Count == 4) {

                            packetLength = BitConverter.ToInt32(recvBuffer.ToArray(), 0);
                            recvBuffer.Clear();
                        }

                    } else if (packetId == -1) {

                        if (recvBuffer.Count == 2) {

                            packetId = BitConverter.ToUInt16(recvBuffer.ToArray(), 0);
                            recvBuffer.Clear();
                        }

                    } else if (recvBuffer.Count == packetLength) {

                        break;
                    }

                } finally { mutex.ReleaseMutex(); }
            }

            onRecv(packetId, recvBuffer.ToArray());
        }

        public void RecvT (Action<int, object> onRecv) {

            byte[] buffer = new byte[1];
            int recvByteCount = 0, packetLength = -1, packetId = -1;
            List<byte> recvBuffer = new List<byte>();

            while (true) {

                mutex.WaitOne(); try {

                    recvByteCount = stream.Read(buffer, 0, 1);
                    recvBuffer.Add(buffer[0]);

                    if (recvByteCount == 0) Close();

                    if (packetLength == -1) {

                        if (recvBuffer.Count == 4) {

                            packetLength = BitConverter.ToInt32(recvBuffer.ToArray(), 0);
                            recvBuffer.Clear();
                        }

                    } else if (packetId == -1) {

                        if (recvBuffer.Count == 2) {

                            packetId = BitConverter.ToUInt16(recvBuffer.ToArray(), 0);
                            recvBuffer.Clear();
                        }

                    } else if (recvBuffer.Count == packetLength) {

                        break;
                    }

                } finally { mutex.ReleaseMutex(); }
            }

            BinaryFormatter binaryFormatter = new BinaryFormatter();
            MemoryStream memoryStream = new MemoryStream();

            memoryStream.Write(recvBuffer.ToArray(), 0, packetLength);
            memoryStream.Seek(0, SeekOrigin.Begin);

            onRecv(packetId, binaryFormatter.Deserialize(memoryStream));
        }
    }
}
